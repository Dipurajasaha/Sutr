...
async def generate_summary(db: AsyncSession, file_id: str, summary_type: str) -> str:
    # -- 1. fetch all text chunks for the given file_id --
    result = await db.execute(
        select(TextChunk.text).where(TextChunk.file_id == file_id).order_by(TextChunk.chunk_index)
    )    
    ...
    # -- 3. define prompt based on summary type --
    prompt_instruction = (
